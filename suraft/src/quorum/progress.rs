//! Progress tracks replication state, i.e., it can be considered a map of node
//! id to already replicated log id.
//!
...
///
/// When one of the value is updated, it uses a `QuorumSet` to calculate the
/// committed value. `ID` is the identifier of every progress value.
/// `V` is type of progress entry.
/// `P` is the progress data of `V`, a progress entry `V` could contain other
/// user data. `QS` is a quorum set implementation.
...
where
    ...
    QS: QuorumSet<ID>,
{
    /// Update one of the scalar value and re-calculate the committed value with
    /// provided function.
    ///
    ...
    /// Get the greatest value that is granted by a quorum defined in
    /// [`Self::quorum_set()`].
    ///
    /// In raft or other distributed consensus,
    /// To commit a value, the value has to be **granted by a quorum** and has
    /// to be the greatest value every proposed.
    #[allow(dead_code)]
    fn granted(&self) -> &P;
...
where
    ...
    QS: QuorumSet<ID>,
{
    /// Quorum set to determine if a set of `id` constitutes a quorum, i.e.,
    /// committed.
    quorum_set: QS,
    ...
    /// Progress data.
    ///
    /// Elements with values greater than the `granted` are sorted in
    /// descending order. Others are unsorted.
    ///
    /// The first `voter_count` elements are voters, the left are learners.
    /// Learner elements are always still.
    /// A voter element will be moved up to keep them in descending order, when
    /// a new value is updated.
    vector: Vec<(ID, V)>,
...
pub struct DisplayVecProgress<'a, ID, V, P, QS, Fmt>
where
    ID: 'static,
    QS: QuorumSet<ID>,
    Fmt: Fn(&mut Formatter<'_>, &ID, &V) -> std::fmt::Result,
{
    inner: &'a VecProgress<ID, V, P, QS>,
    f: Fmt,
}
    ...
    P: Clone,
{
    ...
    /// Move an element at `index` up so that all the values greater than
    ...
    #[allow(dead_code)]
    pub(crate) fn into_iter(self) -> impl Iterator<Item = (ID, V)> {
    ...
    QS: QuorumSet<ID>,
{
    ...
    ///
    /// This way it gets rid of a portion of unnecessary re-calculation of
    /// committed, and avoids unnecessary sorting: progresses are kept in
    /// order and only values greater than committed need to sort.
    ///
        ...
        ID: PartialEq,
    {
        ...
        if prev_le_granted && new_gt_granted {
            ...
            // From high to low, find the max value that has constituted a
            // quorum.
