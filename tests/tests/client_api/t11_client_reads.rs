...
/// - call the ensure_linearizable interface on the followers, and assert
///   failure.
#[tracing::instrument]
...
async fn client_reads() -> Result<()> {
    ...
    router.ensure_linearizable(leader.clone()).await.unwrap_or_else(|_| {
        panic!(
            "ensure_linearizable to succeed for cluster leader {}",
            leader
        )
    });

    router
        .ensure_linearizable(s(1))
        .await
        .expect_err("ensure_linearizable on follower node 1 to fail");
    router
        .ensure_linearizable(s(2))
        .await
        .expect_err("ensure_linearizable on follower node 2 to fail");

    tracing::info!(
        log_index,
        "--- isolate node 1 then ensure_linearizable should work"
    );
    ...
    tracing::info!(
        log_index,
        "--- isolate node 2 then ensure_linearizable should fail"
    );
...
/// - A leader that has not yet committed any log entries returns leader
///   initialization log id(blank log id).
/// - Return the last committed log id if the leader has committed any log
///   entries.
#[tracing::instrument]
#[test_harness::test(harness = ut_harness)]
async fn get_read_log_id() -> Result<()> {
    ...
    tracing::info!(
        log_index = log_index,
        "--- node 1 appends blank log but can not commit"
    );
    {
        let res = n1
            .wait(timeout())
    ...
    tracing::info!("--- stop blocking, write another log, get_read_log_id returns last log id");
    {
        ...
        n1.wait(timeout())
            .applied_index(Some(log_index + 1), "commit blank log")
            .await?;
        log_index += 1;
    ...
    );
    {
        ...
        log_index += 1;
        n1.wait(timeout())
            .log_index(Some(log_index), "log appended, but not committed")
            .await?;
