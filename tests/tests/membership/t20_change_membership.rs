...
/// When a change-membership log is committed, the `RaftState.membership_state`
/// should be updated.
#[tracing::instrument]
...
async fn update_membership_state() -> anyhow::Result<()> {
    ...
    tracing::info!(log_index, "--- change membership from 012 to 01234");
    {
        ...
        let res = leader
            .change_membership([s(0), s(1), s(2), s(3), s(4)], false)
            .await?;
        log_index += 2;

        tracing::info!(
            log_index,
            "--- change_membership blocks until success: {:?}",
            res
        );
        ...
        for node_id in [s(0), s(1), s(2), s(3), s(4)] {
            ...
            router.external_request(node_id, move |st| {
                ...
                assert_eq!(
                    st.membership_state.committed().log_id().index(),
                    Some(log_index)
                );
                assert_eq!(
                    st.membership_state.effective().log_id().index(),
                    Some(log_index)
                );
            });
        }
...
async fn change_with_new_learner_blocking() -> anyhow::Result<()> {
    // Add a member without adding it as learner, in blocking mode it should
    // finish successfully.
    ...
    );
    {
        ...
        match raft_err.api_error().unwrap() {
            ClientWriteError::ChangeMembershipError(
                ChangeMembershipError::LearnerNotFound(err),
            ) => {
