...
use maplit::btreemap;
use maplit::btreeset;
use suraft::errors::ChangeMembershipError;
use suraft::errors::ClientWriteError;
use suraft::errors::InProgress;
...
async fn add_learner_basic() -> Result<()> {
    ...
    tracing::info!(
        log_index,
        "--- re-adding leader commits a new log but does nothing"
    );
    {
        let res = router.add_learner(s(0), s(0)).await?;
        log_index += 1;

        assert_eq!(log_index, res.log_id.index);
        router
            .wait(&s(0), timeout())
            .applied_index(Some(log_index), "commit re-adding leader log")
            .await?;
    }
    ...
    tracing::info!(log_index, "--- add new node node-1");
    {
        ...
        tracing::info!(
            log_index,
            "--- add_learner blocks until the replication catches up"
        );
        {
            let (mut sto1, _sm1) = router.get_storage_handle(&s(1))?;
    ...
    {
        let res = router.add_learner(s(0), s(1)).await?;
        log_index += 1;

        assert_eq!(log_index, res.log_id.index);
        router
            .wait(&s(0), timeout())
            .applied_index(Some(log_index), "commit re-adding node-1 log")
            .await?;

        let metrics = router.get_raft_handle(&s(0))?.metrics().borrow().clone();
        let node_ids = metrics
            .membership_config
            .membership()
            .nodes()
            .map(|x| x.0.clone())
            .collect::<Vec<_>>();
        assert_eq!(vec![s(0), s(1)], node_ids);
    }
...
async fn add_learner_non_blocking() -> Result<()> {
    ...
    tracing::info!(log_index, "--- add new node node-1, in non blocking mode");
    {
        ...
        router
            .client_request_many(s(0), "learner_add", 100 - log_index as usize)
            .await?;
        log_index = 100;

        router
            .wait(&s(0), timeout())
            .applied_index(Some(log_index), "received 100 logs")
            .await?;

        router.new_raft_node(s(1)).await;

        // Replication problem should not block adding-learner in non-blocking
        // mode.
        ...
        for i in 0..=n {
            ...
            let metrics =
                router.get_raft_handle(&s(0))?.metrics().borrow().clone();
            let repl = metrics.replication.as_ref().unwrap();

            // The result is Some(&None) when there is no success replication is
            // made, and is None if no replication attempt is
            // made(no success or failure is reported to metrics).
            let n1_repl = repl.get(&s(1));
            if n1_repl.is_none() {
            ...
            assert_eq!(
                Some(&None),
                n1_repl,
                "no replication state to the learner is reported"
            );
            break;
        }
    }
    ...
    tracing::info!(log_index, "--- set node 2 and 4");
    {
        ...
        raft.change_membership(
            ChangeMembers::SetNodes(
                btreemap! {s(2)=>suraft::emp(), s(4)=>suraft::emp()},
            ),
            true,
        )
        .await?;
...
///
/// Because adding learner is also a change-membership operation, a new
/// membership config log will let raft consider the previous membership config
/// log as committed, which is actually not.
#[tracing::instrument]
#[test_harness::test(harness = ut_harness)]
async fn add_learner_when_previous_membership_not_committed() -> Result<()> {
    ...
    tracing::info!(
        log_index,
        "--- block replication to prevent committing any log"
    );
    {
        router.set_network_error(s(1), true);
        ...
        assert_eq!(
            ClientWriteError::ChangeMembershipError(
                ChangeMembershipError::InProgress(InProgress {
                    ...
                    membership_log_id: Some(log_id(1, log_index + 1))
                })
            ),
            err
        );
...
async fn check_learner_after_leader_transferred() -> Result<()> {
    ...
    let mut log_index =
        router.new_cluster(btreeset! {s(0),s(1)}, btreeset! {s(2)}).await?;

    // Submit a config change which adds two new nodes and removes the current
    // leader.
    let orig_leader_id =
    ...
    assert_eq!(
        s(0),
        orig_leader_id,
        "expected original leader to be node 0"
    );

    router.new_raft_node(s(3)).await;
    router.new_raft_node(s(4)).await;
    router.add_learner(orig_leader_id.clone(), s(3)).await?;
    router.add_learner(orig_leader_id.clone(), s(4)).await?;
    log_index += 2;
    router
        .wait_for_log(
            &btreeset! {s(0), s(1)},
    ...
    node.change_membership([s(1), s(3), s(4)], false).await?;
    ...
    tracing::info!(log_index, "--- old leader commits 2 membership log");
    {
        router
            ...
            .applied_index(
                Some(log_index),
                "old leader commits 2 membership log",
            )
            .await?;
    }
    ...
    tracing::info!(log_index, "--- check new cluster membership");
    {
        ...
        let m =
            StorageHelper::new(&mut sto1, &mut sm1).get_membership().await?;

        // new membership is applied, thus get_membership() only returns one
        // entry.

        assert_eq!(
            &Membership::new(
                vec![btreeset! {s(1), s(3), s(4)}],
                Some(btreeset! {s(2)})
            ),
    ...
    tracing::info!(
        log_index,
        "--- check learner in new cluster can receive new log"
    );
    {
        let new_leader =
            router.leader().expect("expected the cluster to have a new leader");
        router.client_request_many(new_leader, "0", 1).await?;
        log_index += 1;

        for i in [s(1), s(2), s(3), s(4)] {
            router
                .wait(&i, timeout())
                .applied_index_at_least(Some(log_index), "learner recv new log")
                .await?;
        }
    }
