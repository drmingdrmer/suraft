...
use maplit::btreeset;
use suraft::errors::InitializeError;
use suraft::errors::NotAllowed;
use suraft::errors::NotInMembers;
...
/// - asserts that they remain in learner state with no activity (they should be
///   completely passive).
/// - initializes the cluster with membership config including all nodes.
/// - asserts that the cluster was able to come online, elect a leader and
///   maintain a stable state.
/// - asserts that the leader was able to successfully commit its initial
///   payload and that all followers have successfully replicated the payload.
#[tracing::instrument]
...
async fn initialization() -> anyhow::Result<()> {
    ...
    router
        .wait_for_log(&btreeset! {s(0), s(1), s(2)}, None, timeout(), "empty")
        .await?;
    router
        .wait_for_state(
    ...
    //
    // Since the execution of API messages is serialized, even if the request
    // executes some unknown time in the future (due to fire-and-forget
    // semantics), it will properly receive the state before initialization,
    // as that state will appear later in the sequence.
    //
    // Also, this external request will be definitely executed, since it's
    // ordered before other requests in the SuRaft core API queue, which
    // definitely are executed (since they are awaited).
    ...
    // Initialize the cluster, then assert that a stable cluster was formed &
    // held.
    ...
    // At this time, one of the nodes is the leader, all the others are
    // followers. Check via an external request as well. Again, this is not
    // required for the correctness of the test.
    //
    ...
    for node in [s(0), s(1), s(2)] {
        let server_state =
            router.with_raft_state(node.clone(), |s| s.server_state).await?;
        match server_state {
            ServerState::Leader => {
                assert!(!found_leader);
                found_leader = true;
...
async fn initialize_err_target_not_include_target() -> anyhow::Result<()> {
    // Initialize a node with membership config that does not include the target
    // node that accepts the `initialize` request.
