...
/// A restarted unreachable node should recover correctly, and catch up with the
/// leader:
...
async fn append_entries_backoff_rejoin() -> Result<()> {
    ...
    {
        // Timeout leader lease otherwise vote-request will be rejected by
        // node-2
