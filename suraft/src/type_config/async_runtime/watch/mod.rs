...
/// A `Watch` is a pair of `WatchSender` and `WatchReceiver` that can be used to
...
pub trait Watch: Sized + OptionalSend {
    ...
    ///
    /// If the implementation of `Ref` holds a lock on the inner value, it means
    /// that long-lived borrows could cause the producer half to block.
    ///
    /// In practice, it is recommended to keep the borrow as short-lived as
    /// possible. Additionally, if you are running in an environment that
    /// allows `!Send` futures, you must ensure that the returned `Ref` type
    /// is never held alive across an `.await` point, otherwise, it can lead
    /// to a deadlock.
    ///
    /// In particular, a producer which is waiting to acquire the lock
    /// in `send` might or might not block concurrent calls to `borrow_watched`,
    /// e.g.:
    ///
    ...
    ///
    /// All values sent by [`WatchSender`] should become visible to the
