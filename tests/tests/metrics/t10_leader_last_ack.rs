...
async fn leader_last_ack_3_nodes() -> Result<()> {
    ...
    tracing::info!(
        log_index,
        "--- heartbeat; millis_since_quorum_ack refreshes"
    );
    {
...
async fn leader_last_ack_3_nodes_abs_time() -> Result<()> {
    ...
    // Upon receiving a replication Response, it wakes up Core and flush the
    // metrics again. This cause the last_quorum_acked to be updated.
    ...
    tracing::info!(
        log_index,
        "--- sleep 500 ms, the `last_quorum_acked` should not change"
    );
    {
        TypeConfig::sleep(Duration::from_millis(500)).await;
    ...
    tracing::info!(
        log_index,
        "--- sleep and heartbeat again; last_quorum_acked refreshes"
    );
    {
        TypeConfig::sleep(Duration::from_millis(500)).await;
