...
use crate::type_config::async_runtime::instant::Instant;
...
/// impl suraft::AsyncRuntime for MyCustomRuntime { /* omitted */ }
///
/// // Build a runtime
...
impl<Rt: AsyncRuntime> Suite<Rt> {
    ...
    pub async fn test_watch_overwrite_init_value() {
        ...
        // into a block so that they can be dropped before invoking
        // `.borrow_watched()`, which needs an immutable reference to
        // `rx`.
        {
            let mut changed_fut = rx.changed();
            let mut pinned_changed_fut = pin!(changed_fut);
            assert!(matches!(
                poll_in_place(pinned_changed_fut.as_mut()),
                Poll::Pending
            ));
    ...
    pub async fn test_oneshot_send_from_another_task() {
        let number_to_send = 1;
        let (tx, rx) = Rt::Oneshot::channel::<i32>();
        // no need to join the task, this test only works iff the sender task
        // finishes its job
        let _handle = Rt::spawn(async move {
            tx.send(number_to_send).unwrap();
        });
    ...
    pub async fn test_mutex() {
        ...
        drop(guard);
        assert!(matches!(
            poll_in_place(pinned_another_guard_fut),
            Poll::Ready(_)
        ));
    }
}
