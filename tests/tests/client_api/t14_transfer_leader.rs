...
use maplit::btreeset;
use suraft::suraft::TransferLeaderRequest;
use suraft::Config;
...
///
/// Call [`handle_transfer_leader`](suraft::suraft::SuRaft::handle_transfer_leader) on every
...
async fn transfer_leader() -> anyhow::Result<()> {
    ...
    tracing::info!("--- can NOT transfer Leader from 2 to 0 with an old vote");
    {
        ...
        let n0_res = n0
            ...
            .state(
                ServerState::Leader,
                "node-0 can not become leader with old leader vote",
            )
            .await;
...
///
/// Call [`trigger().
/// transfer_leader()`](suraft::suraft::trigger::Trigger::transfer_leader) on
/// the Leader node to force establish a new leader.
#[tracing::instrument]
#[test_harness::test(harness = ut_harness)]
