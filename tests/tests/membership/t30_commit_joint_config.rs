...
/// There is a bug that leader commit a member change log directly because it
/// only checks the replication of voters and believes itself is the only voter.
///
...
/// To reproduce the bug that new config does not actually examine the
/// term/index of learner, but instead only examining the followers
///
/// - bring a cluster of node 0,1,2 online.
/// - isolate 3,4; change config to 2,3,4
/// - since new config can not form a quorum, the joint config should not be
///   committed.
#[tracing::instrument]
#[test_harness::test(harness = ut_harness)]
async fn commit_joint_config_during_012_to_234() -> Result<()> {
    ...
    // the first step of joint should not pass because the new config can not
    // constitute a quorum
