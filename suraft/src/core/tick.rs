//! tick emitter emits a `APIMessage::Tick` event at a certain interval.
...
where C: TypeConfig
{
    ...
    /// Signal the tick loop to stop. And return a JoinHandle to wait for the
    /// loop to stop.
    ///
    ...
    pub(crate) fn shutdown(&self) -> Option<JoinHandleOf<C, ()>> {
        {
            ...
            if let Some(shutdown) = shutdown {
                ...
                tracing::warn!("Double call to SuRaft::shutdown()");
            }
        }
