...
/// inconsistent logs the conflict is always set to `last_log`, which blocks
/// replication for ever. https://github.com/drmingdrmer/suraft/blob/79a39970855d80e1d3b761fadbce140ecf1da59e/async-raft/src/core/append_entries.rs#L131-L154
///
/// - fake a cluster of node 0,1,2. R0 has ~100 uncommitted log at term 2. R2
///   has ~100 uncommitted log at term 3.
///
/// ```
/// R0 ... 2,99 2,100
/// R1
...
async fn append_inconsistent_log() -> Result<()> {
    ...
    // leader appends at least one blank log. There may be more than one
    // transient leaders
    log_index += 1;
