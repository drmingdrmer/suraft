...
//!
//! `async` runtime is an abstraction over different asynchronous runtimes, such
//! as `tokio`, `async-std`, etc.
...
///
/// The intention of this trait is to allow an application using this crate to
/// bind an asynchronous runtime that suits it the best.
///
...
pub trait AsyncRuntime:
    Debug + Default + PartialEq + Eq + OptionalSend + OptionalSync + 'static
{
    ...
    /// The timeout type used by [`Self::timeout`] and [`Self::timeout_at`] that
    /// enables the user to await the outcome of a [`Future`].
