...
async fn remove_leader() -> Result<()> {
    ...
    let mut log_index = router
        .new_cluster(btreeset! {s(0),s(1)}, btreeset! {s(2), s(3)})
        .await?;
    ...
    // Another node(e.g. node-1) in the old cluster may not commit the second
    // membership change log. Because to commit the 2nd log it only need a
    // quorum of the new cluster.
    ...
    tracing::info!(log_index, "--- check state of the old leader");
    {
        let metrics = router
            .wait(&s(0), timeout())
            .state(ServerState::Learner, "old leader steps down")
            .await?;
        let cfg = &metrics.membership_config.membership();

        assert_eq!(metrics.current_term, 1);
        assert_eq!(metrics.committed, Some(8));
        assert_eq!(metrics.last_applied, Some(LogId::new(1, 8)));
        assert_eq!(
            metrics.membership_config.membership().get_joint_config().clone(),
            vec![btreeset! {s(1),s(2),s(3)}]
        );
...
async fn remove_leader_and_convert_to_learner() -> Result<()> {
    ...
    tracing::info!(
        log_index,
        "--- change membership and retain removed node as learner"
    );
    {
    ...
    tracing::info!(
        log_index,
        "--- wait 1 sec, old leader(non-voter) stays as a leader"
    );
    {
...
///
/// It should not respond a ForwardToLeader error that pointing to the removed
/// leader.
#[tracing::instrument]
#[test_harness::test(harness = ut_harness)]
async fn remove_leader_access_new_cluster() -> Result<()> {
    ...
    tracing::info!(log_index, "--- change membership 012 to 2");
    {
        ...
        router
            ...
            // because leader node-0 will steps down at once when the second
    ...
    let res = router
        .send_client_request(s(2), ClientRequest::make_request("foo", 1))
        .await;
    match res {
        Ok(_) => {
    ...
    tracing::info!(log_index, "--- elect node-2, handle write");
    {
        ...
        n2.wait(timeout())
            .state(ServerState::Leader, "node-2 elect itself")
            .await?;
        log_index += 1;
        ...
        n2.wait(timeout())
            .applied_index(
                Some(log_index),
                "node-2 become leader and handle write request",
            )
            .await?;
    }
